...
public class ServerThread extends Thread
{
  ...
  public ServerThread(Socket sock){ // 서버 연결해주기
    try{
      st_sock = sock; // 소켓 초기화
  ...
  // 버퍼 길이 측정 → 메시지/제목 등 출력 → 정보/코드번호 출력 
  
  private void sendErrCode(int message, int errCode) throws IOException{
    st_buffer.setLength(0);
    st_buffer.append(message); // 에러 메시지 출력
    st_buffer.append(SEPARATOR); // 구분자
    st_buffer.append(errCode); // 에러 코드
    send(st_buffer.toString());
  }

  private void modifyWaitRoom() throws IOException{ // 대기실 생성
    st_buffer.setLength(0);
